...
export function PoolPage() {
  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-8 sm:px-9">
      ...
      {isLoading ? (
        <p className="text-[15px] text-secondary">Loading pool…</p>
      ) : isError ? (
        <p className="text-[15px] text-danger">Couldn’t load the pool. Please try again.</p>
      ) : !questions || questions.length === 0 ? (
        <div className="rounded-[10px] border border-default bg-surface p-4">
          <p className="mb-1 text-[14px] font-semibold text-primary">The pool is empty</p>
          <p className="text-[14px] text-secondary">
  ...
  return (
    <article className="rounded-[10px] border border-default bg-surface p-3.5">
      <div className="mb-2 flex items-start justify-between gap-3">
        <p className="text-[14px] font-semibold text-primary">{question.stem}</p>
        {badge ? (
          <span
      ...
      {question.explanation ? (
        <p className="mb-2 text-[13px] leading-snug text-muted">{question.explanation}</p>
      ) : null}
